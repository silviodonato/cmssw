...
class PixelJetPuId : public edm::EDFilter {
   ...
   private:
     ...
     edm::InputTag m_trackIPTagInfos;
...
PixelJetPuId::PixelJetPuId(const edm::ParameterSet& iConfig)
{
  ...
  m_trackIPTagInfos   = iConfig.getParameter<edm::InputTag>("trackIPTagInfos");
...
PixelJetPuId::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   //get TrackIPTagInfo
   Handle<std::vector<reco::TrackIPTagInfo> > trackIPTagInfos;
   iEvent.getByLabel(m_trackIPTagInfos, trackIPTagInfos);
   ...
   for( ; taginfo != trackIPTagInfos->end() && it != jetTracksAssociation->end(); taginfo++, i++, it++ ) {
	i++;
	int ntracks=0;
	const std::vector< reco::TrackIPTagInfo::TrackIPData >& tracksIPdata = taginfo->impactParameterData();
	...
	std::vector< reco::TrackIPTagInfo::TrackIPData >::const_iterator itTrackData = tracksIPdata.begin();      
	//loop on tracks
	for(; itTrack != tracks.end() &&  itTrackData != tracksIPdata.end(); ++itTrack, ++itTrackData) 
	{
		//select the tracks compabible with the jet
		if(( (*itTrack)->pt() > m_MinTrackPt) && ((*itTrackData).distanceToJetAxis.value()>-m_MaxTrackDistanceToJet) && ( (*itTrack)->normalizedChi2() < m_MaxTrackChi2))
		{
        		ntracks++;
